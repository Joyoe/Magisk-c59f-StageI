...
object JokerHide : BaseSettingsItem.Toggle() {
    override val title = R.string.jokerhide.asText()
    override val description = R.string.settings_jokerhide_summary.asText()
    override var value = Config.jokerHide
        set(value) = setV(value, field, { field = it }) {
            val cmd = if (it) "enable" else "disable"
            Shell.jojo("jokerhide $cmd").submit { cb ->
                if (cb.isSuccess) Config.jokerHide = it
                else field = !it
            }
        }
}
